...
##                                                                                        ##
##                                                                                        ##
## Part 3: Susceptibility of Daphnia as response variable (entire population)             ##  
##                                                                                        ##
...
myfile <- read_excel("Raw data Ilic, Cordellier and Fink, Freshwater Biology.xlsx", sheet = "Susceptibility")
...
##                 Negative value indicates that the somatic growth rate              ##
##                 of Daphnia was higher in presence of the corresponding PUFA        ##
...
myfile <- subset(myfile, PUFA != "C")  ## exclude control treatment from this part of the analysis (only relevant for single clones)
myfile <- droplevels(myfile)

myfile$PUFA <- factor(myfile$PUFA, levels = c("ALA","EPA","ARA"))
...
shapiro.test(myfile$Susceptibility)   ## normal distribution not given
par(mfrow = c(1,2))
hist(myfile$Susceptibility, las = 1)
qqPlot(myfile$Susceptibility, distribution = "norm", las = 1)

##===============================================
## Check the homogeneity of variances ----

leveneTest(Susceptibility ~ PUFA, data = myfile)          ## homogeneious (equal) variances
leveneTest(Susceptibility ~ Genotype, data = myfile)      ## heterogeneious (unequal) variances
leveneTest(Susceptibility ~ PUFA*Genotype, data = myfile) ## homogeneious variances
...
screen(2)
par(mar = c(6,5,1,1))
boxplot(Susceptibility ~ PUFA*Genotype, data = myfile, las = 2, xlab = "")
screen(3)
par(mar = c(4,5,1,1))
boxplot(Susceptibility ~ PUFA, data = myfile, las = 1)
screen(4)
par(mar = c(4,5,1,1))
boxplot(Susceptibility ~ Genotype, data = myfile, las = 2)

##===============================================
## Null model: linear mixed-effects model (LME) ----

# Response variable: Susceptibility (of Daphnia)
# Fixed effects: PUFA
#                Genotype
...
Model.null <- lme(Susceptibility ~ PUFA*Genotype, 
                  random = ~1|Day,
                  control = ctrl,
                  data = myfile,
                  method = "REML")
...
Model.1 <- lme(Susceptibility ~ PUFA*Genotype, 
               random = ~1|Day,
               weights = vf1,
               control = ctrl,
               data = myfile, 
...
Model.2 <- lme(Susceptibility ~ PUFA*Genotype, 
               random = ~1|Day,
               weights = vf2,
               control = ctrl,
               data = myfile,
...
Model.3 <- lme(Susceptibility ~ PUFA*Genotype, 
               random = ~1|Day,
               weights = vf3,
               control = ctrl,
               data = myfile, 
...
cor(myfile$Susceptibility, fitted(Model.3))^2
...
shapiro.test(mylongi$Susceptibility)   ## normal distribution not given
dev.off()
par(mfrow = c(1,2))
hist(mylongi$Susceptibility, las = 1)
qqPlot(mylongi$Susceptibility, distribution = "norm", las = 1)

##===============================================
## Check the homogeneity of variances ----

leveneTest(Susceptibility ~ PUFA, data = mylongi)          ## homogeneous (equal) variances
leveneTest(Susceptibility ~ Genotype, data = mylongi)      ## heterogeneious (unequal) variances
leveneTest(Susceptibility ~ PUFA*Genotype, data = mylongi) ## homogeneious variances
...
screen(2)
par(mar = c(6,5,1,1))
boxplot(Susceptibility ~ PUFA*Genotype, data = mylongi, las = 2, xlab = "")
screen(3)
par(mar = c(4,5,1,1))
boxplot(Susceptibility ~ PUFA, data = mylongi, las = 1)
screen(4)
par(mar = c(4,5,1,1))
boxplot(Susceptibility ~ Genotype, data = mylongi, las = 2)

##===============================================
## Null model: linear mixed-effects model (LME) ----

# Response variable: Susceptibility (somatic growth rate of Daphnia)
# Fixed effects: PUFA
#                Genotype
#                Interaction PUFA x Genotype
# Random effect: Time, given in days (Day)
...
shapiro.test(E.3) ## normally distributed
dev.off()
par(mfrow = c(1,1))
hist(E.3,las = 1)
...
cor(mylongi$Susceptibility, fitted(Model.3))^2
...
# Average susceptibility for each clone in each treatment

avg.clone <- myfile %>% 
        group_by(Genotype,PUFA) %>% 
        summarize(Mean.clone = mean(Susceptibility, na.rm = T),
                  SD.clone = sd(Susceptibility, na.rm = T))

avg.clone

# Average susceptibility of the entire population in each treatment
