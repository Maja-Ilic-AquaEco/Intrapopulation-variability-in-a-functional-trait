...
##                                                                                        ##
##                                                                                        ##
## Part 2: Somatic growth rate of Daphnia as response variable (individual clones)        ##  
##                                                                                        ##
...
df <- read_excel("Raw data Ilic, Cordellier and Fink, Freshwater Biology.xlsx", sheet = "SGR")
head(df)
str(df)
...
# Fit a linear mixed-effects model for each genotype with Growth_rate as response variable, 
# PUFA as fixed effect and Day of the year as random effect

lme_SGR <- df %>%
  dplyr::select(Day,Genotype,PUFA,Growth_rate) %>% 
  nest(-Genotype) %>% 
  mutate(
    fit = map(data, ~ lme(Growth_rate ~ PUFA,
                          random = ~ 1|Day,
                          data = .x)),
    tidied = map(fit, tidy),
...
dir.create("ggHistNorm Residuals SGR")
dir.create("ggQQplot Residuals SGR")
dir.create("Variance homogeneity Residuals SGR")
dir.create("Homoscedasticity for SGR")
...
for(i in 1:length(genotypes)){
  ...
  ggHistNorm.resid <- ggHistNorm(mygenotype,mygenotype$Residuals,paste("Standardized Residuals for SGR - Genotype",genotypes[i]),0.5)[[1]]
  
  ggsave(paste0("ggHistNorm Residuals SGR/Genotype ",genotypes[i],".png"),
         ggHistNorm.resid, width = 10, height = 8)
  
  ggQQplot.resid <- ggQQplot(mygenotype,mygenotype$Residuals,paste("Standardized Residuals for SGR - Genotype",genotypes[i]))
  
  ggsave(paste0("ggQQplot Residuals SGR/Genotype ",genotypes[i],".png"),
  ...
  ggsave(paste0("Variance homogeneity Residuals SGR/Genotype ",genotypes[i],".png"),
  ...
  ggsave(paste0("Homoscedasticity for SGR/Genotype ",genotypes[i],".png"),
...
write.table(results.df, "LME model asumptions and output for each genotype - SGR.csv", sep = ",", row.names = F)

write.table(results.final, "Dunnett posthoc test for each genotype - SGR.csv", sep = ",", row.names = F)